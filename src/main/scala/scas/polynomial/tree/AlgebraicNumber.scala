...
class AlgebraicNumber[C, @specialized(Int, Long) N](val ring: Field[C], val pp: PowerProduct[N], val list: List[Element[C, N]], val module: Module[Polynomial.Element[C, N], C, N])(implicit val cm: ClassManifest[Element[C, N]]) extends TreePolynomial[Element[C, N], C, N] with scas.polynomial.AlgebraicNumber[Polynomial.Element, Element[C, N], C, N] {
  def this(ring: Field[C], pp: PowerProduct[N], list: List[Element[C, N]]) = this(ring, pp, list, Module("e", 2, Polynomial(ring, pp)))
  def this(ring: Field[C], pp: PowerProduct[N]) = this(ring, pp, List())
  def this(mod: Element[C, N]) = this(mod.factory.ring, mod.factory.pp, List(mod))
...
object AlgebraicNumber {
  ...
  def apply[C, @specialized(Int, Long) N](mod: Element[C, N]) = new AlgebraicNumber(mod)
