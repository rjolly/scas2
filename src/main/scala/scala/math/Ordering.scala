...
trait Ordering[@specialized(Int, Long) T] extends Comparator[T] with PartialOrdering[T] with Serializable {
  outer =>
 ...
 /** Returns an integer whose sign communicates how x compares to y.
  ...
  /** This implicit method augments `T` with the comparison operators defined
    ...
    */
  implicit def mkOrderingOps(lhs: T): Ops[T] = new OpsImpl(lhs, outer)
}
...
object Ordering extends LowPriorityOrderingImplicits {
  ...
  trait ExtraImplicits {
    ...
    /** This implicit creates a conversion from any value for which an
      ...
      * }}}
      */
    implicit def infixOrderingOps[@specialized(scala.Int, scala.Long) T: Ordering](x: T) = implicitly[Ordering[T]].mkOrderingOps(x)
  }

  /** This inner class defines comparison operators available for `T`. */
  trait Ops[@specialized(scala.Int, scala.Long) T] {
    val lhs: T
    val factory: Ordering[T]
    def <(rhs: T) = factory.lt(lhs, rhs)
    def <=(rhs: T) = factory.lteq(lhs, rhs)
    def >(rhs: T) = factory.gt(lhs, rhs)
    def >=(rhs: T) = factory.gteq(lhs, rhs)
    def equiv(rhs: T) = factory.equiv(lhs, rhs)
    def max(rhs: T): T = factory.max(lhs, rhs)
    def min(rhs: T): T = factory.min(lhs, rhs)
  }

  class OpsImpl[@specialized(scala.Int, scala.Long) T](val lhs: T, val factory: Ordering[T]) extends Ops[T]
